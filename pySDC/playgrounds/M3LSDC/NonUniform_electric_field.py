...
# noinspection PyUnusedLocal
class non_uniformElectric_generic_implicit(Problem):
    r"""
    This class implements a standard Penning trap problem on the time interval :math:`[0, t_{end}]`
    fully investigated in [1]_. The equations are given by the following equation of motion
    ...
    def eval_f(self, u, t):
        ...
        f[1]=u[4]
        f[2]=u[5]
        f[3]=(1/self.omega_B)*0-c*u[0]
        f[4]=(1/self.omega_B)*u[5]+(c/2)*u[1]
        f[5]=-(1/self.omega_B)*u[4]+(c/2)*u[2]
    ...
    def fuct(self, x, dt, rhs):
        ...
        f[1]=x[4]
        f[2]=x[5]
        f[3]=(1/self.omega_B)*0-c*x[0]
        f[4]=(1/self.omega_B)*x[5]+(c/2)*x[1]
        f[5]=-(1/self.omega_B)*x[4]+(c/2)*x[2]
